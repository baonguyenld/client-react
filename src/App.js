...
const [isLog,setIsLog]= useState(localStorage.getItem('log') == null ?  false : true );
  return (
    <div>
      <Router>
        <Routes>
          {routes.map((route) => {
            ...
            const category = route.category != undefined ? route.category : undefined;
              ...
              <Route
                ...
                element={
                  <Layout flag ={flag} setFlag={setFlag} isLog={isLog}>
                    <Page category={category} type ={type} list={list} log={log} setFlag={setFlag} setIsLog={setIsLog} choice={choice}/>
                  </Layout>
                }
              />
            );
